module.exports = {
  getProducts: async (req, res) => {
    ...
    const result = await db.products.getAllProducts();
  ...
  inputProduct: async (req, res) => {
    const { id, product_name, product_price, product_quantity, product_image } =
      req.body;
    const db = req.app.get("db");
    const date = new Date();
    const { userid } = req.session.user || 0;
    const newItem = await db.products.inputProduct([
      id,
      product_name,
      product_price,
      product_quantity,
