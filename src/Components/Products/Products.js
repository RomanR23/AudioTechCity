...
function Products() {
  ...
  const onAdd = (product) => {
    if (product.product_quantity > 0) {
      ...
      if (exist) {
        ...
        axios.post("/api/updateExistingProduct", product).then(() => {
          product.product_quantity = 0;
          getCartItems();
        });
      } else {
        axios.post("/api/inputProduct", product).then(() => {
          product.product_quantity = 0;
          getCartItems();
        });
      }
    } else {
      alert("Please Select The Amount First!");
    }
  };
  ...
  return (
    <div className="products-container-main">
      <div className="products-content-box">{array}</div>
    </div>
  );
}
