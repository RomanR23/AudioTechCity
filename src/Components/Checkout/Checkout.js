...
function Checkout() {
	...
	const checkoutRemove = (product) => {
		axios.post('/api/deleteProductCheckout', product).then((_) => getCartItems());
	};
		...
		return (
			<div className="product-checkout-indv" key={index}>
				...
				<div className="product-counter-checkout">
					<button className="checkout-product-quantity-button" onClick={() => checkoutDec(product)}>
						-
					</button>
					<div>{product.product_quantity}</div>
					<button className="checkout-product-quantity-button" onClick={() => checkoutInc(product)}>
						+
					</button>
					<button className="remove-item-button" onClick={() => checkoutRemove(product)}>
						Remove Item
					</button>
				</div>
			</div>
			...
			<div className="checkout-products-info-box">
				...
				<div className="checkout-product-payment-container">
					...
					<div className="checkout-payment-bottom">
						...
						<button className="checkout-pay-button" id="button" onClick={() => checkoutPay()}>
							Checkout & Pay
						</button>
					</div>
				</div>
